...
app.post("/login",  (request, response) => {
    ...
    db.query(searchCommand, [user.email], (error, data) => {
        ...
        if(data.length === 0) {
            response.json({ message: "Não tem nenhum usuario cadastrado com esse e-mail!"})
            return
        }
        ...
        response.json({ message: "Credenciais invalidas! Tente novamente"})
    })
})
...
app.get("/verify", (request, response) => {
    ...
    jwt.verify(token, SECRET_KEY, (error,decoded) => {
        ...
        console.log('Id do usuario: ${decoded.id}')
        console.log('Email do usuario: ${decoded.email}')
